...
PROTOCOL_MAP = {
    ...
    "dns": 5, "ftp": 6, "ssh": 7, "smtp": 8, "other": 9,
    "unknown": 9, "nan": 9, "6": 0, "17": 1, "1": 2,
}

STATE_MAP = {
    "established": 0, "syn_sent": 1, "syn_recv": 2, "fin_wait": 3,
    "close_wait": 4, "closed": 5, "unknown": 6, "nan": 6,
    "con": 0, "int": 1, "fin": 3, "req": 1, "rsp": 2,
}
...
class Preprocessor:
    ...
    def fit_transform(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, list]:
        """
        Clean + encode + normalize the canonical DataFrame.
        ...
        df["protocol_enc"] = (
            df["protocol"].astype(str).str.lower().str.strip()
            .map(lambda x: PROTOCOL_MAP.get(x, 9))
        )
        df["state_enc"] = (
            df["connection_state"].astype(str).str.lower().str.strip()
            .map(lambda x: STATE_MAP.get(x, 6))
        )
